...
bsed_uniform_bootstrap <- community_bootstrap(community_df = communities_energy[[1]], bootstrap_function = 'bootstrap_unif_bsed_doi', nbootstraps = 100)

```

    - **RMD There are two plausible interpretations of this step. If you compare the "true uniform distribution" as _energy use_ is 1/n_sizeclasses, any community of uniform _sizes_ will deviate from that and the $DOI$s will actually be more like 1 or 2. If you compare the uniform distribution as the energy use distribution if every size class has an equal amount of biomass, that'll be a little different. First I am doing the equal energy in all size classes, but if this is weird it will be good to go back here and try it with energy calculated from truly uniform biomass allocation to size classes.**
    
    
```{r plot BSED-uniform bootstrap DOIs v empirical} 

bsed_uniform_bootstrap_plot <- plot_bsed_bootstrap_results(bsed_uniform_bootstrap, names(communities_energy)[[1]])

bsed_uniform_bootstrap_plot
```
