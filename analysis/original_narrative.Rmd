...
ernest_bsds_uniform_results = read.csv(here::here('ernest-2005-files/ernest_appendixA.csv')) %>%
...
```{r ernest ks two sample results}
...
```{r print ks two sample}
for(i in 1:length(bsd_crosscomm_ks)){
  print(bsd_crosscomm_ks[[i]]$ks_result$p.value)
}
```
```{r ks two sample log}
...
bsd_crosscomm_ks_log = lapply(community_combinations, FUN = ks_bsd, 
                          ln_mass_vals = T)
```

```{r print ks two sample log}
for(i in 1:length(bsd_crosscomm_ks_log)){
  print(bsd_crosscomm_ks_log[[i]]$ks_result$p.value)
}
```
