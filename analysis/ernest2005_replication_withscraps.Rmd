...
if(source_sims) {
  load(file.path('data', 'sims', 'bsed_uniform_bootstraps.Rdata'))
} else {
  
bsed_uniform_bootstraps <- lapply(communities, FUN = community_bootstrap,  bootstrap_function = 'bootstrap_unif_bsed_doi', nbootstraps = 10000)

save(bsed_uniform_bootstraps, file = (file.path('data', 'sims', 'bsed_uniform_bootstraps.Rdata')))
}
```
...
if(source_sims) {
  load(file.path('data', 'sims', 'bsed_crosscomm_bootstraps.Rdata'))
} else {
  
bsed_crosscomm_bootstraps = lapply(community_combinations, FUN = community_bootstrap, 
                                   bootstrap_function = 'bootstrap_crosscomm_bseds', nbootstraps = 10000)

save(bsed_crosscomm_bootstraps, file = (file.path('data', 'sims', 'bsed_crosscomm_bootstraps.Rdata')))
}
```
...
write.csv(ernest_key, file = file.path('ernest-2005-files/ernest_key.csv'), row.names = F)
```
...
ernest_bsds_uniform_results = read.csv(file.path('ernest-2005-files/ernest_appendixA.csv'), stringsAsFactors = F) %>%
