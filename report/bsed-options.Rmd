...
Trying a sampling scheme not on log size energy unit scale:
```{r true uniform size energy not log}

true_uniform_bsed <- data.frame(individual_sizes = seq(andrews_vars$min_size, andrews_vars$max_size, by = .1), individual_species = "notimpt") %>%
  add_energy_sizeclass() %>%
  dplyr::mutate(individual_energy = andrews_vars$total_energy / andrews_vars$nind) %>%
  make_bsed()

plot_bsed(true_uniform_bsed)
```
