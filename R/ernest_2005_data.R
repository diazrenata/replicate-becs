...
download_raw_paper_data <- function() {

  if(!dir.exists(here::here('data', 'paper', 'raw','andrews-lter'))) {
    dir.create(here::here('data', 'paper', 'raw','andrews-lter'))
  }
  download.file('http://andlter.forestry.oregonstate.edu/ltermeta/ltersearch/dataaccess.aspx?docid=WE02601_v1.csv', here::here('data', 'paper', 'raw','andrews-lter', 'andrews.csv'))
  download.file('http://andlter.forestry.oregonstate.edu/mdaccess/metadownload.aspx?dbcode=WE026', here::here('data', 'paper', 'raw','andrews-lter', 'andrews-metadata.pdf'))
  download.file('http://andlter.forestry.oregonstate.edu/ltermeta/ltersearch/dataaccess.aspx?docid=SA00501_v2.csv', here::here('data', 'paper', 'raw','andrews-lter', 'andrews-specieslist.csv'))


  if(!dir.exists(here::here('data', 'paper', 'raw','niwot'))) {

    dir.create(here::here('data', 'paper', 'raw','niwot'))

  }

  download.file('http://niwot.colorado.edu/data_csvs/smammals.jh.data.csv', here::here('data', 'paper', 'raw','niwot', 'niwot-raw.csv'))
  download.file('http://niwot.colorado.edu/meta_data/smammals.jh.meta.txt', here::here('data', 'paper', 'raw','niwot', 'niwot-metadata.txt'))



  if(!dir.exists(here::here('data', 'paper', 'raw','sev'))) {

    dir.create(here::here('data', 'paper', 'raw','sev'))

  }

  download.file('https://portal.lternet.edu/nis/dataviewer?packageid=knb-lter-sev.8.297976&entityid=d70c7027949ca1d8ae053eb10300dc0e', here::here('data', 'paper', 'raw','sev', 'sev.csv'))
  download.file('https://portal.lternet.edu/nis/metadataviewer?packageid=knb-lter-sev.8.297976', here::here('data', 'paper', 'raw','sev','sev-metadata.html'))
...
process_andrews_data <- function() {
  ...
  andrews_raw <- read.csv(here::here('data', 'paper', 'raw', 'andrews-lter', 'andrews.csv'), stringsAsFactors = F)
  ...
  andrews_species <-  read.csv(here::here('data', 'paper', 'raw', 'andrews-lter', 'andrews-specieslist.csv'), stringsAsFactors = F)
  ...
  processedpath = here::here('data', 'paper', 'processed')
  if(!dir.exists(processedpath)) {
    dir.create(processedpath)
  }

  write.csv(andrews, here::here('data', 'paper', 'processed', 'andrews-processed.csv'), row.names = F)
...
process_niwot_data <- function(datapath = here::here()){
  ...
  niwot_raw <- read.csv(here::here('data','paper', 'raw', 'niwot', 'niwot-raw.csv'), stringsAsFactors = F)
  ...
  write.csv(niwot, here::here('data','paper', 'processed', 'niwot-processed.csv'), row.names = F)
...
process_sev_data <- function(){
  ...
  #
  infile1  <- here::here('data','paper', 'raw', 'sev', 'sev.csv')
  ...
  for(i in 1:length(sev_locations)) {
    ...
    write.csv(this_sev, here::here('data','paper', 'processed',paste0('sev-', this_location, '-processed.csv')), row.names = F)
...
process_portal_data <- function(portaldatapath = '/Users/renatadiaz/Documents/GitHub/weecology/') {
  ...
  write.csv(portal, here::here('data', 'paper', 'processed', 'portal-processed.csv'), row.names = F)
