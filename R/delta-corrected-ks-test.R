...
#' @description  delta-corrected KS test from Zar 1999, comparing a focal distribution to a uniform distribution.
#' @param distribution distribution to compare to uniform
...
#' @param n_or_i "n" or "i" see issue on GitHub
                        ...
                        expected_range = NULL, n_or_i = "n")
{
  ...
  if(delta_correction) {
    ...
    d_crits = read.csv(file.path(system.file(package= "replicatebecs"), "data", "ks-critical-values", "delta_kstable.csv"), stringsAsFactors = F)
    ...
    d_crits = read.csv(file.path(system.file(package= "replicatebecs"), "data", "ks-critical-values", "kstable.csv"), stringsAsFactors = F)
...
#' @title Find KS p-values
#' @description helper for ks test
#' @param rel_f rel_f table from zar_ks_test()
#' @param d_crits d_crit table from zar_ks_test()
#' @param deltaval 0, 1, or null for non-delta-corrected
#' @param nvals sample size, or NULL in which case i is used
#' @return list of minimum and maximum p values
